...
async def examrules(ctx):
    ...
    if ctx.channel == cmdChan:
        ...
        embed.add_field(name="Do not ask questions", value="Users found asking questions during the exam will receive one warning. Should they continue to ask questions, they will be banned and <@&685609130785898510> will be notified.", inline=False)
...
async def rules(ctx):
    ...
    if ctx.channel == cmdChan:
        ...
        embed.add_field(name="5. No shitposting/memes" , value="An occaisonal post is fine, but please try to limit it severely.", inline=False)
        ...
        embed.add_field(name="9. Do not DM Offensive Securit Staff", value="You may only DM <@&685609130785898510> with their explicit consent.", inline=False)
        embed.add_field(name="10. No illegal activities", value="This includes cracking of passwords that aren't yours, asking for pirated software, or compromised accounts.", inline=False)
        embed.add_field(name="11. Do not ignore the rules", value="People blatantly ignoring the rules will either get warned or kicked out from the server", inline=False)
